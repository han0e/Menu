...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        ...
        alert('유효하지 않은 링크이거나 세션이 만료되었습니다. 다시 시도해주세요.');
        navigate('/login');
      }
    });
  }, [navigate]);
  ...
  if (!sessionInfo) {
    return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: 'var(--bg)', color: 'var(--gold-bright)' }}>Loading...</div>;
  }
